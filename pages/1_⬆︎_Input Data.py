...
with st.form("My"):
    ...
    with col1:
        d = st.date_input("Issue Date", datetime.date(2023, 6, 3))
        item = st.text_input('Item', '')
        ghatpcs = st.number_input('Ghat PCs')
        pahad = st.number_input('Pahad Weight')
        kundan = st.number_input('Kundan Weight')

    with col2:
        d1 = st.date_input("Receive Date", datetime.date(2023, 6, 3))
        jadiyaname = st.text_input('Jadiya Name', '')
        jobn = st.number_input('Job number', step=1)
